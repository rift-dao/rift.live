import { css } from 'styled-components';

export const palette = {
  riftGray: '#19191d',
  successGreen: '#419c5f',
  destructiveOrange: '#af4026',
};

export const colors = {
  bgAlphaBlack: '#0007',
  bgAlphaWhite: '#fff5',
  bg: palette.riftGray,
  bgSecondary: '#bbb',
  interactiveBg: '#303540',
  primary: '#ddd',
  primaryActive: 'white',
  secondary: palette.riftGray,
  secondaryActive: 'black',
  interactiveDefault: '#eeea',
  confirm: palette.successGreen,
  confirmActive: '#51a86a',
  destructive: palette.destructiveOrange,
  destructiveActive: '#c54935',
  warning: '#f0f',
  accentColor: '#f4b',
};

export const interactiveGlow = (color, textShadowColor = null) => css`
  color: ${color};
  opacity: 0.8;

  &:hover {
    text-shadow: 0 0 5px ${textShadowColor || color};
    opacity: 1;
  }
`;

export const colorBorderColor = (color) =>
  `color: ${color}; border: 1px solid ${color};`;

export const boxShadow = (depth) =>
  `box-shadow: 1px ${depth * 2}px ${((depth * 2) / 3) * 2}px 0 #1116;`;

export const dialogBorder = (usePurple = false) => css`
  background-color: ${usePurple ? '#382a67' : colors.interactiveBg};
  border: solid;
  /* border-radius: 45px; */
  box-shadow: inset -5px -5px 0px 5px #fff4, inset 5px 5px 0px 5px #0008;
  border-image: url('/images/dialog-borderbox${usePurple ? '_purple' : ''}.png')
    28 / 10px 10px 10px 10px stretch;
  color: #ccc;
  text-shadow: -1px -1px 0 #aaa1, 2px 1px 1px #0006;
  text-transform: uppercase;
  image-rendering: pixelated;
`;

export const repeatingGradient = (
  angle,
  color1,
  color1Thickness = 10,
  color2 = '#0000',
  color2Thickness = 0,
) => css`
  repeating-linear-gradient(${angle}, ${color1}, ${color1} ${color1Thickness}px, ${color2} ${color1Thickness}px, ${color2} ${
  color2Thickness || color1Thickness
})
`;

export const noiseImg = `url('data:image/png;base64,')`;
const gradPosition = '100% 0';
const gradStart = '25%';
const gradStop = '65%';

export const radialGradientItem = (position, start, stop, color) => css`
  background: repeat-x ${position} / 50% 100%
    radial-gradient(
      circle at center center,
      ${color} ${start},
      transparent ${stop}
    );
`;

export const itemUnicolor = (color) => css`
  color: ${color};
  &::before {
    ${radialGradientItem(gradPosition, gradStart, gradStop, color)}
  }
`;

export const dither = (position, start, stop, color) => css`
  background: radial-gradient(
    circle at ${position},
    transparent ${start},
    ${color} ${stop}
  );
  mask: ${noiseImg},
    radial-gradient(
      circle at ${position},
      transparent ${start},
      #000 (${stop} + 10%)
    );
`;

export const fonts = {
  dijit: "dijit, retro, 'Press Start 2P', 'Courier Prime', monospace",
  game: "'Press Start 2P', 'Courier Prime', monospace",
  retro: "retro, 'Press Start 2P', 'Courier Prime', monospace",
  mono: "'Courier Prime', monospace",
  sans: "'Source Sans Pro', sans-serif",
};

export const gameFont = css`
  font-family: ${fonts.retro};
  -webkit-font-smoothing: none;
  -moz-osx-font-smoothing: none;
  font-smoothing: none;
  font-smooth: never;
  image-rendering: pixelated;
  transform: perspective(1px) translateZ(0);
  backface-visibility: hidden;
`;

export default {
  colors,
  breakpoints: ['40em', '52em', '64em'],
  defaultStyles: {
    borderRadius: '3px 0 2px 0',
  },
  fonts,
  zIndex: {
    background: 50,
    base: 100,
    overlay: 500,
    modal: 1000,
  },
  buttons: {
    primary: {
      color: colors.interactiveDefault,
    },
    secondary: {
      color: colors.bg,
    },
  },
};

/**
 * Using z-index
 *
 * Layers:
 *
 * 90: Rift Logo
 * 100: Main Content
 * 200: Social
 * 1000: Modals
 */
